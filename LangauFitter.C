...
std::vector <TGraphErrors*> PEmean;
//TGraphErrors *PEmean = new TGraphErrors();
...
void LangauFitter()
{
  ...
  Double_t param=0.5;     //Change based on what is being scanned
  Double_t param_step=0.5; //Increment for the horizontal axis
  TGraphErrors *gr;
  ...
  while(std::getline(rfiles, line)){
    ...
    cout << line.data() << endl;
    tmpStr = line.data();
    tmpStr = tmpStr.ReplaceAll("plots/300MeV_Greater_cut/","");//wherever your files happen to be

    param_run = param + counter*param_step;
    tmp = (TH1D*)file->Get("ThetaX_pes");  //Loads a histogram associated with a ring of the user's choice
    
    hst = (TH1D*)tmp->Clone("Langau PEs");
    hst->SetTitle("R5 PE yield for ThetaX normal and ThetaY varied, Mmuon Energy > 300 MeV");
    ...
    m = FindGraph();
    PEmean[m]->SetPoint(PEmean[m]->GetN(),param_run,fitP[1]);
    PEmean[m]->SetPointError(PEmean[m]->GetN()-1,0.1,fitE[1]);

    hst->SaveAs(Form("plots/Langau/300MeV_Greater_cut/Langau_%s",tmpStr.Data()));
    
    counter = counter + 1.0;
    file->Close("R");  
  }
  C_mp->Update();
  C_mp->SaveAs("plots/300MeV_Greater_cut/R5_ThetaX_normal_ThetaY_varied.root");
  rfiles.close();
}
...
Int_t FindGraph()
{
   ...
   for(int k = 0; k < PEmean.size(); k++){
      if(PEmean[k]){
         name = PEmean[k]->GetName();
   ...
   gr->SetName("PEMean_angle");
   gr->SetMarkerStyle(21);
   //gr->GetYaxis()->SetRangeUser(0,35);
   PEmean.push_back(gr);
   C_mp->cd();
   gr->Draw("AP");
   gr->GetXaxis()->SetTitle("Angle of incidence (+- 0.1) [degrees]"); //Change based on what is being scanned
   gr->GetYaxis()->SetTitle("PE yield");
   gr->SetTitle("PE yield variation with ThetaY, Muon Energy > 300 MeV");
   return PEmean.size()-1;
