...
#sa_stop = 0
#sa_step = 5
sa = 0           #0.0872665  #Value of sa is hard set rather than looped over. Done so that it could be a double with units
...
cutx_start = 0.0  #Used for hr = 10. Selects a section of the full segment to scan over (bounds will be determined later) in 10 mm increments (can be adjusted). 0 is the first 10 mm of R1.
cutx_stop = 0.0   #Currently repurposed to move scintillator around
cutx_step = 0.1
                ...
                for cutx in np.arange(cutx_start,cutx_stop+cutx_step,cutx_step):
                    for cuty in np.arange(cuty_start,cuty_stop+cuty_step,cuty_step):
                        for tilt in np.arange(tilt_start,tilt_stop+tilt_step,tilt_step):
                            ...
                            Text = ""
                            FileIDString = "_hR"+str(hr)+"_cutx"+str(round(cutx,2))+"_cuty"+str(round(cuty,2))+"_det"+str(det)+"_tilt"+str(round(tilt,2))
