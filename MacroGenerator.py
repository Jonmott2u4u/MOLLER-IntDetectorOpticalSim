...
hr_start = 8    #See macros for hr info
hr_stop = 8
hr_step = 1     #Increments over each value of hr

cut_start = 0  #Breaks the quartz tiles up into multiple segments. Only works for EventRegion == 8,9. Currently not looped over. If it needs to be used, add to loop
cut_stop = 19
cut_step = 1
        ...
        for id in np.arange(ID_start,ID_stop+ID_step,ID_step):
            for det in np.arange(det_start,det_stop+det_step,det_step):
                for cut in np.arange(cut_start,cut_stop+cut_step,cut_step):
                    ...
                    Text = ""
                    FileIDString = "_sa"+str(sa)+"_hR"+str(hr)+"_det"+str(det)+"_cut"+str(cut)
                    if det == 1:
