...
void ExtractScanResults()
{
  ...
  ring_dat.open ("r2.txt"); //Change the name to match the ring being analyzed, otherwise files will be overwritten
  ring_dat_weighted.open ("r2_w.txt");
  ...
  while(std::getline(rfiles, line)){
    ...
    tmp = (TH1D*)file->Get("R2_CathodeEventsDistrHist");  //Loads a histogram associated with a ring of the user's choice
    ...
    tmp2 = (TH1D*)file->Get("R3_CathodeEventsDistrHist");  //Loads a histogram associated with a ring of the user's choice
