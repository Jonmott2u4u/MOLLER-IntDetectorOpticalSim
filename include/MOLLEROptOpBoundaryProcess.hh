    ...
    if (surface) {
      ...
      if (optical_surface) {
        ...
        if (sMPT) {
	      ...
	      //G4cout << "incident_angle = " << incident_angle << ", angle_key = " << angle_key.first << G4endl;         DOES NOT WORK. [] DOES NOT SAVE INFO?
              return angle_key.first > value;
            });
            
          if (lower_bound != angles_keys.end()){
            const auto property_table = sMPT->GetProperty(lower_bound->second);
	    //G4cout << "2nd loop: incident_angle = " << incident_angle << ", angle_key = " << angle_key.first << G4endl; DOES NOT WORK. [] DOES NOT SAVE INFO?
