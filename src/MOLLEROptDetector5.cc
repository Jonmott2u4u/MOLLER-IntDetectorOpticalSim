...
MOLLEROptDetector5::MOLLEROptDetector5(MOLLEROptTrackingReadout5 *TrRO5, G4String type, MOLLEROptMaterial* mat)
{	    	    
  ...
  Quartz = new  MOLLEROptDetectorQuartz5(TrackingReadout5,type,Materials);
  LightGuide = new  MOLLEROptDetectorLightGuide5(TrackingReadout5,type,Materials);
  PMT = new  MOLLEROptDetectorPMT5(TrackingReadout5,type,Materials,LightGuide);     
  detMessenger5 = NULL;
  ...
  fReadFile ="../data/December5051/GDML/Ring5/R5-Module-Al.gdml";
...
MOLLEROptDetector5::~MOLLEROptDetector5()
{
  delete Quartz;
  delete LightGuide;
  delete PMT;
...
void MOLLEROptDetector5::SetQuartzSizeX(G4double x)
{
  if(Quartz)
    Quartz->SetQuartzSizeX(x);
}

void MOLLEROptDetector5::SetQuartzSizeY(G4double y)
{
  if(Quartz)
    Quartz->SetQuartzSizeY(y);
}
				       
void MOLLEROptDetector5::SetQuartzSizeZ(G4double z)
{
  if(Quartz)
...
void MOLLEROptDetector5::SetQuartzRotX(G4double rX)
{
  if(Quartz)
...
void MOLLEROptDetector5::SetLowerInterfacePlane(G4double LowerPlane)
{
  if(LightGuide)
    LightGuide->SetLowerInterfacePlane(LowerPlane);
}

void MOLLEROptDetector5::SetMiddleBoxHeight(G4double MiddlePlane)
{
  if(LightGuide)
    LightGuide->SetMiddleBoxHeight(MiddlePlane);
}

void MOLLEROptDetector5::SetUpperInterfacePlane(G4double UpperPlane)
{
  if(LightGuide)
    LightGuide->SetUpperInterfacePlane(UpperPlane); 
}

void MOLLEROptDetector5::SetLowerConeFrontFaceAngle(G4double angle)
{
  if(LightGuide)
    LightGuide->SetLowerConeFrontFaceAngle(angle);
}

void MOLLEROptDetector5::SetLowerConeBackFaceAngle(G4double angle)
{
  if(LightGuide)
    LightGuide->SetLowerConeBackFaceAngle(angle);
}

void MOLLEROptDetector5::SetLowerConeSideFaceAngle(G4double angle)
{  
  if(LightGuide){
...
void MOLLEROptDetector5::SetQuartzInterfaceOpeningZ(G4double size)
{
  if(LightGuide)
    LightGuide->SetQuartzInterfaceOpeningZ(size);
}

void MOLLEROptDetector5::SetQuartzInterfaceOpeningX(G4double size)
{
  if(LightGuide)
    LightGuide->SetQuartzInterfaceOpeningX(size);
}

void MOLLEROptDetector5::SetPMTInterfaceOpeningZ(G4double size)
{
  if(LightGuide)
...
void MOLLEROptDetector5::SetPMTInterfaceOpeningX(G4double size)
{
  if(LightGuide)
...
void MOLLEROptDetector5::SetQuartzToPMTOffsetInZ(G4double val)
{
...
void MOLLEROptDetector5::SetAzimuthalRotationAngle(G4double val)
{
  AzimuAngle = val; 
}

void MOLLEROptDetector5::SetPolarRotationAngle(G4double val)
{
  PolarAngle = val;

}

void MOLLEROptDetector5::SetPMTCathodeThickness(G4double val)
{
  if(PMT)
    PMT->SetCathodeThickness(val);
}

void MOLLEROptDetector5::SetPMTCathodeRadius(G4double val)
{
  if(PMT)
...
void MOLLEROptDetector5::UpdateThisGeometry()
{
...
void MOLLEROptDetector5::CalculateDimensions()
{
...
void MOLLEROptDetector5::ResetCenterLocation()
{
  PositionDetX = 0.0*cm;
  PositionDetY = -116.3*cm;//DetFullLengthY/2 - Quartz->GetQuartzSizeY()/2;
  PositionDetZ = 96.9*cm;
...
void MOLLEROptDetector5::Initialize()
{
  ...
  // Quartz->SetCenterPositionInY(-0.5*DetFullLengthY+quartzY/5.0 + 5*mm);
  ...
  // PMT->SetCenterPositionInY(-0.5*DetFullLengthY+quartzY+lguideY+PMT->GetPMTLength()/5.0 + 5.0*mm);// + 0.5*LightGuide->GetCurrentQuartzInterfaceOpeningY()*TMath::Sin(Qrot));
...
G4VPhysicalVolume* MOLLEROptDetector5::ConstructDetector(G4VPhysicalVolume* Mother)
{
  if(!Mother) return 0;
  ...
  G4Colour  grey      ( 157/555., 157/555., 157/555.);
  ...
  if(!detMessenger5)
    detMessenger5 = new MOLLEROptDetectorMessenger5(this);  


  return DetPhysical;
} 

void MOLLEROptDetector5::ConstructMountingStructure(G4VPhysicalVolume* Mother)
{
  ...
  // cout << "Quartz center position Y: " << Quartz->GetCurrentCenterPositionInY() << "Set = " << -0.5*DetFullLengthY+quartzY/5.0 + 5*mm << endl;
...
void MOLLEROptDetector5::SetMaterial(G4String materialName)
{
  DetMaterial = Materials->GetMaterial(materialName);
}

void MOLLEROptDetector5::SetCenterPositionInX(G4double xPos)
{
    PositionDetX = xPos;	 
...
void MOLLEROptDetector5::SetCenterPositionInY(G4double yPos)
{
    PositionDetY = yPos;
...
void MOLLEROptDetector5::SetCenterPositionInZ(G4double zPos)
{
    PositionDetZ = zPos;
...
void MOLLEROptDetector5::SetLightGuideOffsetInX(G4double x)
{
  G4double cX;
...
void MOLLEROptDetector5::SetLightGuideOffsetInY(G4double y)
{
  G4double cY;
...
void MOLLEROptDetector5::SetLightGuideOffsetInZ(G4double z)
{
  G4double cZ;
...
void MOLLEROptDetector5::GetQuartzLimits(G4double *vals)
{
  Quartz->GetQuartzLimits(vals);
}

void MOLLEROptDetector5::GetLightGuideLimits(G4double *vals)
{
