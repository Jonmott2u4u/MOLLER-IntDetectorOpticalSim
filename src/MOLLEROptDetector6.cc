...
MOLLEROptDetector6::MOLLEROptDetector6(MOLLEROptTrackingReadout6 *TrRO6, G4String type, MOLLEROptMaterial* mat)
{	    	    
  ...
  Quartz = new  MOLLEROptDetectorQuartz6(TrackingReadout6,type,Materials);
  LightGuide = new  MOLLEROptDetectorLightGuide6(TrackingReadout6,type,Materials);
  PMT = new  MOLLEROptDetectorPMT6(TrackingReadout6,type,Materials,LightGuide);     
  detMessenger6 = NULL;
  ...
  fReadFile ="../data/December6061/GDML/Ring5/R5-Module-Al.gdml";
...
MOLLEROptDetector6::~MOLLEROptDetector6()
{
  delete Quartz;
  delete LightGuide;
  delete PMT;
...
void MOLLEROptDetector6::SetQuartzSizeX(G4double x)
{
  if(Quartz)
    Quartz->SetQuartzSizeX(x);
}

void MOLLEROptDetector6::SetQuartzSizeY(G4double y)
{
  if(Quartz)
    Quartz->SetQuartzSizeY(y);
}
				       
void MOLLEROptDetector6::SetQuartzSizeZ(G4double z)
{
  if(Quartz)
...
void MOLLEROptDetector6::SetQuartzRotX(G4double rX)
{
  if(Quartz)
...
void MOLLEROptDetector6::SetLowerInterfacePlane(G4double LowerPlane)
{
  if(LightGuide)
    LightGuide->SetLowerInterfacePlane(LowerPlane);
}

void MOLLEROptDetector6::SetMiddleBoxHeight(G4double MiddlePlane)
{
  if(LightGuide)
    LightGuide->SetMiddleBoxHeight(MiddlePlane);
}

void MOLLEROptDetector6::SetUpperInterfacePlane(G4double UpperPlane)
{
  if(LightGuide)
    LightGuide->SetUpperInterfacePlane(UpperPlane); 
}

void MOLLEROptDetector6::SetLowerConeFrontFaceAngle(G4double angle)
{
  if(LightGuide)
    LightGuide->SetLowerConeFrontFaceAngle(angle);
}

void MOLLEROptDetector6::SetLowerConeBackFaceAngle(G4double angle)
{
  if(LightGuide)
    LightGuide->SetLowerConeBackFaceAngle(angle);
}

void MOLLEROptDetector6::SetLowerConeSideFaceAngle(G4double angle)
{  
  if(LightGuide){
...
void MOLLEROptDetector6::SetQuartzInterfaceOpeningZ(G4double size)
{
  if(LightGuide)
    LightGuide->SetQuartzInterfaceOpeningZ(size);
}

void MOLLEROptDetector6::SetQuartzInterfaceOpeningX(G4double size)
{
  if(LightGuide)
    LightGuide->SetQuartzInterfaceOpeningX(size);
}

void MOLLEROptDetector6::SetPMTInterfaceOpeningZ(G4double size)
{
  if(LightGuide)
...
void MOLLEROptDetector6::SetPMTInterfaceOpeningX(G4double size)
{
  if(LightGuide)
...
void MOLLEROptDetector6::SetQuartzToPMTOffsetInZ(G4double val)
{
...
void MOLLEROptDetector6::SetAzimuthalRotationAngle(G4double val)
{
  AzimuAngle = val; 
}

void MOLLEROptDetector6::SetPolarRotationAngle(G4double val)
{
  PolarAngle = val;

}

void MOLLEROptDetector6::SetPMTCathodeThickness(G4double val)
{
  if(PMT)
    PMT->SetCathodeThickness(val);
}

void MOLLEROptDetector6::SetPMTCathodeRadius(G4double val)
{
  if(PMT)
...
void MOLLEROptDetector6::UpdateThisGeometry()
{
...
void MOLLEROptDetector6::CalculateDimensions()
{
...
void MOLLEROptDetector6::ResetCenterLocation()
{
  PositionDetX = -8.7*cm;
  PositionDetY = -116.5*cm;//DetFullLengthY/2 - Quartz->GetQuartzSizeY()/2;
...
void MOLLEROptDetector6::Initialize()
{
  ...
  // Quartz->SetCenterPositionInY(-0.5*DetFullLengthY+quartzY/6.0 + 5*mm);
  ...
  // PMT->SetCenterPositionInY(-0.5*DetFullLengthY+quartzY+lguideY+PMT->GetPMTLength()/6.0 + 5.0*mm);// + 0.5*LightGuide->GetCurrentQuartzInterfaceOpeningY()*TMath::Sin(Qrot));
...
G4VPhysicalVolume* MOLLEROptDetector6::ConstructDetector(G4VPhysicalVolume* Mother)
{
  if(!Mother) return 0;
  ...
  G4Colour  grey      ( 167/655., 167/655., 167/655.);
  ...
  if(!detMessenger6)
    detMessenger6 = new MOLLEROptDetectorMessenger6(this);  


  return DetPhysical;
} 

void MOLLEROptDetector6::ConstructMountingStructure(G4VPhysicalVolume* Mother)
{
  ...
  // cout << "Quartz center position Y: " << Quartz->GetCurrentCenterPositionInY() << "Set = " << -0.5*DetFullLengthY+quartzY/6.0 + 5*mm << endl;
...
void MOLLEROptDetector6::SetMaterial(G4String materialName)
{
  DetMaterial = Materials->GetMaterial(materialName);
}

void MOLLEROptDetector6::SetCenterPositionInX(G4double xPos)
{
    PositionDetX = xPos;	 
...
void MOLLEROptDetector6::SetCenterPositionInY(G4double yPos)
{
    PositionDetY = yPos;
...
void MOLLEROptDetector6::SetCenterPositionInZ(G4double zPos)
{
    PositionDetZ = zPos;
...
void MOLLEROptDetector6::SetLightGuideOffsetInX(G4double x)
{
  G4double cX;
...
void MOLLEROptDetector6::SetLightGuideOffsetInY(G4double y)
{
  G4double cY;
...
void MOLLEROptDetector6::SetLightGuideOffsetInZ(G4double z)
{
  G4double cZ;
...
void MOLLEROptDetector6::GetQuartzLimits(G4double *vals)
{
  Quartz->GetQuartzLimits(vals);
}

void MOLLEROptDetector6::GetLightGuideLimits(G4double *vals)
{
