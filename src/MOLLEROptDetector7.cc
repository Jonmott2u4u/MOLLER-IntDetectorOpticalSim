...
{	    	    
  ...
  fReadFile ="../data/December7071/GDML/Ring5/R5-Module-Al.gdml";
...
MOLLEROptDetector7::~MOLLEROptDetector7()
{
  delete Quartz;
  delete LightGuide;
  delete PMT;
...
void MOLLEROptDetector7::SetQuartzSizeX(G4double x)
{
  if(Quartz)
    Quartz->SetQuartzSizeX(x);
}

void MOLLEROptDetector7::SetQuartzSizeY(G4double y)
{
  if(Quartz)
    Quartz->SetQuartzSizeY(y);
}
				       
void MOLLEROptDetector7::SetQuartzSizeZ(G4double z)
{
  if(Quartz)
...
void MOLLEROptDetector7::SetQuartzRotX(G4double rX)
{
  if(Quartz)
...
void MOLLEROptDetector7::SetLowerInterfacePlane(G4double LowerPlane)
{
  if(LightGuide)
    LightGuide->SetLowerInterfacePlane(LowerPlane);
}

void MOLLEROptDetector7::SetMiddleBoxHeight(G4double MiddlePlane)
{
  if(LightGuide)
    LightGuide->SetMiddleBoxHeight(MiddlePlane);
}

void MOLLEROptDetector7::SetUpperInterfacePlane(G4double UpperPlane)
{
  if(LightGuide)
    LightGuide->SetUpperInterfacePlane(UpperPlane); 
}

void MOLLEROptDetector7::SetLowerConeFrontFaceAngle(G4double angle)
{
  if(LightGuide)
    LightGuide->SetLowerConeFrontFaceAngle(angle);
}

void MOLLEROptDetector7::SetLowerConeBackFaceAngle(G4double angle)
{
  if(LightGuide)
    LightGuide->SetLowerConeBackFaceAngle(angle);
}

void MOLLEROptDetector7::SetLowerConeSideFaceAngle(G4double angle)
{  
  if(LightGuide){
...
void MOLLEROptDetector7::SetQuartzInterfaceOpeningZ(G4double size)
{
  if(LightGuide)
    LightGuide->SetQuartzInterfaceOpeningZ(size);
}

void MOLLEROptDetector7::SetQuartzInterfaceOpeningX(G4double size)
{
  if(LightGuide)
    LightGuide->SetQuartzInterfaceOpeningX(size);
}

void MOLLEROptDetector7::SetPMTInterfaceOpeningZ(G4double size)
{
  if(LightGuide)
...
void MOLLEROptDetector7::SetPMTInterfaceOpeningX(G4double size)
{
  if(LightGuide)
...
void MOLLEROptDetector7::SetQuartzToPMTOffsetInZ(G4double val)
{
...
void MOLLEROptDetector7::SetAzimuthalRotationAngle(G4double val)
{
  AzimuAngle = val; 
}

void MOLLEROptDetector7::SetPolarRotationAngle(G4double val)
{
  PolarAngle = val;

}

void MOLLEROptDetector7::SetPMTCathodeThickness(G4double val)
{
  if(PMT)
    PMT->SetCathodeThickness(val);
}

void MOLLEROptDetector7::SetPMTCathodeRadius(G4double val)
{
  if(PMT)
...
void MOLLEROptDetector7::UpdateThisGeometry()
{
...
void MOLLEROptDetector7::CalculateDimensions()
{
...
void MOLLEROptDetector7::Initialize()
{
  ...
  // Quartz->SetCenterPositionInY(-0.5*DetFullLengthY+quartzY/7.0 + 5*mm);
  ...
  // PMT->SetCenterPositionInY(-0.5*DetFullLengthY+quartzY+lguideY+PMT->GetPMTLength()/7.0 + 5.0*mm);// + 0.5*LightGuide->GetCurrentQuartzInterfaceOpeningY()*TMath::Sin(Qrot));
...
G4VPhysicalVolume* MOLLEROptDetector7::ConstructDetector(G4VPhysicalVolume* Mother)
{
  if(!Mother) return 0;
  ...
  G4Colour  grey      ( 177/755., 177/755., 177/755.);
  ...
  if(!detMessenger7)
    detMessenger7 = new MOLLEROptDetectorMessenger7(this);  


  return DetPhysical;
} 

void MOLLEROptDetector7::ConstructMountingStructure(G4VPhysicalVolume* Mother)
{
  ...
  // cout << "Quartz center position Y: " << Quartz->GetCurrentCenterPositionInY() << "Set = " << -0.5*DetFullLengthY+quartzY/7.0 + 5*mm << endl;
...
void MOLLEROptDetector7::SetMaterial(G4String materialName)
{
  DetMaterial = Materials->GetMaterial(materialName);
}

void MOLLEROptDetector7::SetCenterPositionInX(G4double xPos)
{
    PositionDetX = xPos;	 
...
void MOLLEROptDetector7::SetCenterPositionInY(G4double yPos)
{
    PositionDetY = yPos;
...
void MOLLEROptDetector7::SetCenterPositionInZ(G4double zPos)
{
    PositionDetZ = zPos;
...
void MOLLEROptDetector7::SetLightGuideOffsetInX(G4double x)
{
  G4double cX;
...
void MOLLEROptDetector7::SetLightGuideOffsetInY(G4double y)
{
  G4double cY;
...
void MOLLEROptDetector7::SetLightGuideOffsetInZ(G4double z)
{
  G4double cZ;
...
void MOLLEROptDetector7::GetQuartzLimits(G4double *vals)
{
  Quartz->GetQuartzLimits(vals);
}

void MOLLEROptDetector7::GetLightGuideLimits(G4double *vals)
{
