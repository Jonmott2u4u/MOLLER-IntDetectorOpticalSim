...
MOLLEROptEventAction2::MOLLEROptEventAction2(MOLLEROptAnalysis2* AN, MOLLEROptTrackingReadout2* TrRO2)
{
  analysis2    = AN;
...
MOLLEROptEventAction2::~MOLLEROptEventAction2()
{
  oFile.close();
}


void MOLLEROptEventAction2::BeginOfEventAction(const G4Event* evt)
{
...
Bool_t MOLLEROptEventAction2::TrackHasBeenProcessed(G4int id)
{
  Bool_t check = false;
...
void MOLLEROptEventAction2::AddQuartzTrackStep(G4int id, G4int step)
{
  for(int n = 0; n < ProcessedTrack.size(); n++){
  ...
  lTrackData2 *data = new lTrackData2;
  data->ID = id;
  data->Length = step;
  data->Steps = 1;
...
void MOLLEROptEventAction2::EndOfEventAction(const G4Event* evt)
{

  Initialize();
  analysis2->MOLLERMainEvent->MOLLERPrimEvent.Initialize(); 
  ...
  G4int NumTracks = TrackingReadout2->GetNumTracks();
  ...
  for(int t = 0; t < NumTracks; t++){
    ...
    analysis2->MOLLERMainEvent->MOLLERDetectorEvent.Initialize();  

    track  = TrackingReadout2->GetTrackData2(t);
    if(track){

      analysis2->MOLLERMainEvent->MOLLERDetectorEvent.SetEventID(evt->GetEventID());
      analysis2->MOLLERMainEvent->MOLLERDetectorEvent.SetTrackParentID(track->ParentID);      
      analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddTrackInitMomDirection(track->InitMomDirX,track->InitMomDirY,
									      track->InitMomDirZ);      
      
      if(track->Particle == myElectron){
	analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddElectronTrackID(track->ID);
	analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddPhotonTrackID(0);

	if(track->QuartzHitFlag){
	  analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzTrackHit(1);
	  analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzHitPositionX((Float_t)track->QuartzHitX/cm);
	  analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzHitPositionY((Float_t)track->QuartzHitY/cm);
	}	
	...
	for(int p = 0; p < track->StepNChPhotons.size(); p++){
	  ...
	  analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzStepNPhotons(track->StepNChPhotons[p]);
	  analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzElectronStepLength(track->StepLength[p]/cm);
	}
	for(int p = 0; p < track->SecPhotonAngle.size(); p++)
	  analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzTrackSecPhotonAngle(track->SecPhotonAngle[p]);
      }
      if(track->Particle == myPhoton){
	analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddPhotonTrackID(track->ID);	
	analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddElectronTrackID(0);
	analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzTrackData(track->QLength/cm, track->QSteps);
	analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzPhotonEnergy(track->InitKinEnergy/eV);
	analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzPhotonAtExitFlag(track->QExitFlag);
	analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideTrackData(track->LGLength/cm, track->LGSteps);
	analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTTrackHit(track->PMTHitFlag);
	analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideTrackHit(track->LGHitFlag);
	analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTPhotonEnergy(track->InitKinEnergy/eV);
	analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuidePhotonEnergy(track->InitKinEnergy/eV);
	...
	if(track->PMTHitFlag){
	  ...
	  analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTHitPositionX((Float_t)track->PMTHitX/cm);
	  analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTHitPositionY((Float_t)track->PMTHitY/cm);
	  analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTWindowReflectionAngle((Float_t)track->PMTWinRefl);
	  analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTPhotonEnergy(track->InitKinEnergy/eV);
	  optPhEng = track->InitKinEnergy/eV;
	  OptParam* op = TrackingReadout2->GetOpticalParameters();
	...
	for(int s = 0; s < track->NSteps; s++){
	  if(track->StepVolume[s] == myQuartz){
	    analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzStepLength(track->StepLength[s]/cm);
	    analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzIncidentPhotonAngle(track->StepAngle[s]);
	  }
	  if(track->StepVolume[s] == myLightGuide){
	    analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideStepLength(track->StepLength[s]/cm);
	    analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideIncidentPhotonAngle(track->StepAngle[s]);
	  }
	}
	
      }
    }
    analysis2->FillRootNtuple();
  }

  analysis2->AddPhotoElectronEvent(PMTPe);
  analysis2->AddCathodeDetectionEvent(TrackingReadout2->GetCathodeDetections());
  
  for(int n = 1; n <= QuartzSecOptPhotonCnt->GetNbinsX(); n++)
    analysis2->AddToAverageQuartzOptPhotonDist(QuartzSecOptPhotonCnt->GetBinCenter(n),QuartzSecOptPhotonCnt->GetBinContent(n));
  for(int n = 1; n <= LightGuideSecOptPhotonCnt->GetNbinsX(); n++)
    analysis2->AddToAverageLightGuideOptPhotonDist(LightGuideSecOptPhotonCnt->GetBinCenter(n),LightGuideSecOptPhotonCnt->GetBinContent(n));
  for(int n = 1; n <= PMTSecOptPhotonCnt->GetNbinsX(); n++)
    analysis2->AddToAveragePMTOptPhotonDist(PMTSecOptPhotonCnt->GetBinCenter(n),PMTSecOptPhotonCnt->GetBinContent(n));
  ...
  if (eventNumber%10000 == 1) analysis2->AutoSaveRootNtuple();
  //=======================================================================
  
  
} // end of  MOLLEROptEventAction::EndOfEventAction()

void MOLLEROptEventAction2::Initialize()
{	  
