...
MOLLEROptEventAction3::MOLLEROptEventAction3(MOLLEROptAnalysis3* AN, MOLLEROptTrackingReadout3* TrRO3)
{
  analysis3    = AN;
...
MOLLEROptEventAction3::~MOLLEROptEventAction3()
{
  oFile.close();
}


void MOLLEROptEventAction3::BeginOfEventAction(const G4Event* evt)
{
...
Bool_t MOLLEROptEventAction3::TrackHasBeenProcessed(G4int id)
{
  Bool_t check = false;
...
void MOLLEROptEventAction3::AddQuartzTrackStep(G4int id, G4int step)
{
  for(int n = 0; n < ProcessedTrack.size(); n++){
  ...
  lTrackData3 *data = new lTrackData3;
  data->ID = id;
  data->Length = step;
  data->Steps = 1;
...
void MOLLEROptEventAction3::EndOfEventAction(const G4Event* evt)
{

  Initialize();
  analysis3->MOLLERMainEvent->MOLLERPrimEvent.Initialize(); 
  ...
  G4int NumTracks = TrackingReadout3->GetNumTracks();
  ...
  for(int t = 0; t < NumTracks; t++){
    ...
    analysis3->MOLLERMainEvent->MOLLERDetectorEvent.Initialize();  

    track  = TrackingReadout3->GetTrackData3(t);
    if(track){

      analysis3->MOLLERMainEvent->MOLLERDetectorEvent.SetEventID(evt->GetEventID());
      analysis3->MOLLERMainEvent->MOLLERDetectorEvent.SetTrackParentID(track->ParentID);      
      analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddTrackInitMomDirection(track->InitMomDirX,track->InitMomDirY,
									      track->InitMomDirZ);      
      
      if(track->Particle == myElectron){
	analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddElectronTrackID(track->ID);
	analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddPhotonTrackID(0);

	if(track->QuartzHitFlag){
	  analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzTrackHit(1);
	  analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzHitPositionX((Float_t)track->QuartzHitX/cm);
	  analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzHitPositionY((Float_t)track->QuartzHitY/cm);
	}	
	...
	for(int p = 0; p < track->StepNChPhotons.size(); p++){
	  ...
	  analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzStepNPhotons(track->StepNChPhotons[p]);
	  analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzElectronStepLength(track->StepLength[p]/cm);
	}
	for(int p = 0; p < track->SecPhotonAngle.size(); p++)
	  analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzTrackSecPhotonAngle(track->SecPhotonAngle[p]);
      }
      if(track->Particle == myPhoton){
	analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddPhotonTrackID(track->ID);	
	analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddElectronTrackID(0);
	analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzTrackData(track->QLength/cm, track->QSteps);
	analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzPhotonEnergy(track->InitKinEnergy/eV);
	analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzPhotonAtExitFlag(track->QExitFlag);
	analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideTrackData(track->LGLength/cm, track->LGSteps);
	analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTTrackHit(track->PMTHitFlag);
	analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideTrackHit(track->LGHitFlag);
	analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTPhotonEnergy(track->InitKinEnergy/eV);
	analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuidePhotonEnergy(track->InitKinEnergy/eV);
	...
	if(track->PMTHitFlag){
	  ...
	  analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTHitPositionX((Float_t)track->PMTHitX/cm);
	  analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTHitPositionY((Float_t)track->PMTHitY/cm);
	  analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTWindowReflectionAngle((Float_t)track->PMTWinRefl);
	  analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTPhotonEnergy(track->InitKinEnergy/eV);
	  optPhEng = track->InitKinEnergy/eV;
	  OptParam* op = TrackingReadout3->GetOpticalParameters();
	...
	for(int s = 0; s < track->NSteps; s++){
	  if(track->StepVolume[s] == myQuartz){
	    analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzStepLength(track->StepLength[s]/cm);
	    analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzIncidentPhotonAngle(track->StepAngle[s]);
	  }
	  if(track->StepVolume[s] == myLightGuide){
	    analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideStepLength(track->StepLength[s]/cm);
	    analysis3->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideIncidentPhotonAngle(track->StepAngle[s]);
	  }
	}
	
      }
    }
    analysis3->FillRootNtuple();
  }

  analysis3->AddPhotoElectronEvent(PMTPe);
  analysis3->AddCathodeDetectionEvent(TrackingReadout3->GetCathodeDetections());
  
  for(int n = 1; n <= QuartzSecOptPhotonCnt->GetNbinsX(); n++)
    analysis3->AddToAverageQuartzOptPhotonDist(QuartzSecOptPhotonCnt->GetBinCenter(n),QuartzSecOptPhotonCnt->GetBinContent(n));
  for(int n = 1; n <= LightGuideSecOptPhotonCnt->GetNbinsX(); n++)
    analysis3->AddToAverageLightGuideOptPhotonDist(LightGuideSecOptPhotonCnt->GetBinCenter(n),LightGuideSecOptPhotonCnt->GetBinContent(n));
  for(int n = 1; n <= PMTSecOptPhotonCnt->GetNbinsX(); n++)
    analysis3->AddToAveragePMTOptPhotonDist(PMTSecOptPhotonCnt->GetBinCenter(n),PMTSecOptPhotonCnt->GetBinContent(n));
  ...
  if (eventNumber%10000 == 1) analysis3->AutoSaveRootNtuple();
  //=======================================================================
  
  
} // end of  MOLLEROptEventAction::EndOfEventAction()

void MOLLEROptEventAction3::Initialize()
{	  
