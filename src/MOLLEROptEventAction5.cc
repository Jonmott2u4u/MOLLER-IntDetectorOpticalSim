...
MOLLEROptEventAction5::MOLLEROptEventAction5(MOLLEROptAnalysis5* AN, MOLLEROptTrackingReadout5* TrRO5)
{
  analysis5    = AN;
...
MOLLEROptEventAction5::~MOLLEROptEventAction5()
{
  oFile.close();
}


void MOLLEROptEventAction5::BeginOfEventAction(const G4Event* evt)
{
...
Bool_t MOLLEROptEventAction5::TrackHasBeenProcessed(G4int id)
{
  Bool_t check = false;
...
void MOLLEROptEventAction5::AddQuartzTrackStep(G4int id, G4int step)
{
  for(int n = 0; n < ProcessedTrack.size(); n++){
  ...
  lTrackData5 *data = new lTrackData5;
  data->ID = id;
  data->Length = step;
  data->Steps = 1;
...
void MOLLEROptEventAction5::EndOfEventAction(const G4Event* evt)
{

  Initialize();
  analysis5->MOLLERMainEvent->MOLLERPrimEvent.Initialize(); 
  ...
  G4int NumTracks = TrackingReadout5->GetNumTracks();
  ...
  for(int t = 0; t < NumTracks; t++){
    ...
    analysis5->MOLLERMainEvent->MOLLERDetectorEvent.Initialize();  

    track  = TrackingReadout5->GetTrackData5(t);
    if(track){

      analysis5->MOLLERMainEvent->MOLLERDetectorEvent.SetEventID(evt->GetEventID());
      analysis5->MOLLERMainEvent->MOLLERDetectorEvent.SetTrackParentID(track->ParentID);      
      analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddTrackInitMomDirection(track->InitMomDirX,track->InitMomDirY,
									      track->InitMomDirZ);      
      
      if(track->Particle == myElectron){
	analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddElectronTrackID(track->ID);
	analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddPhotonTrackID(0);

	if(track->QuartzHitFlag){
	  analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzTrackHit(1);
	  analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzHitPositionX((Float_t)track->QuartzHitX/cm);
	  analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzHitPositionY((Float_t)track->QuartzHitY/cm);
	}	
	...
	for(int p = 0; p < track->StepNChPhotons.size(); p++){
	  ...
	  analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzStepNPhotons(track->StepNChPhotons[p]);
	  analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzElectronStepLength(track->StepLength[p]/cm);
	}
	for(int p = 0; p < track->SecPhotonAngle.size(); p++)
	  analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzTrackSecPhotonAngle(track->SecPhotonAngle[p]);
      }
      if(track->Particle == myPhoton){
	analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddPhotonTrackID(track->ID);	
	analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddElectronTrackID(0);
	analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzTrackData(track->QLength/cm, track->QSteps);
	analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzPhotonEnergy(track->InitKinEnergy/eV);
	analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzPhotonAtExitFlag(track->QExitFlag);
	analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideTrackData(track->LGLength/cm, track->LGSteps);
	analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTTrackHit(track->PMTHitFlag);
	analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideTrackHit(track->LGHitFlag);
	analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTPhotonEnergy(track->InitKinEnergy/eV);
	analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuidePhotonEnergy(track->InitKinEnergy/eV);
	...
	if(track->PMTHitFlag){
	  ...
	  analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTHitPositionX((Float_t)track->PMTHitX/cm);
	  analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTHitPositionY((Float_t)track->PMTHitY/cm);
	  analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTWindowReflectionAngle((Float_t)track->PMTWinRefl);
	  analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTPhotonEnergy(track->InitKinEnergy/eV);
	  optPhEng = track->InitKinEnergy/eV;
	  OptParam* op = TrackingReadout5->GetOpticalParameters();
	...
	for(int s = 0; s < track->NSteps; s++){
	  if(track->StepVolume[s] == myQuartz){
	    analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzStepLength(track->StepLength[s]/cm);
	    analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzIncidentPhotonAngle(track->StepAngle[s]);
	  }
	  if(track->StepVolume[s] == myLightGuide){
	    analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideStepLength(track->StepLength[s]/cm);
	    analysis5->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideIncidentPhotonAngle(track->StepAngle[s]);
	  }
	}
	
      }
    }
    analysis5->FillRootNtuple();
  }

  analysis5->AddPhotoElectronEvent(PMTPe);
  analysis5->AddCathodeDetectionEvent(TrackingReadout5->GetCathodeDetections());
  
  for(int n = 1; n <= QuartzSecOptPhotonCnt->GetNbinsX(); n++)
    analysis5->AddToAverageQuartzOptPhotonDist(QuartzSecOptPhotonCnt->GetBinCenter(n),QuartzSecOptPhotonCnt->GetBinContent(n));
  for(int n = 1; n <= LightGuideSecOptPhotonCnt->GetNbinsX(); n++)
    analysis5->AddToAverageLightGuideOptPhotonDist(LightGuideSecOptPhotonCnt->GetBinCenter(n),LightGuideSecOptPhotonCnt->GetBinContent(n));
  for(int n = 1; n <= PMTSecOptPhotonCnt->GetNbinsX(); n++)
    analysis5->AddToAveragePMTOptPhotonDist(PMTSecOptPhotonCnt->GetBinCenter(n),PMTSecOptPhotonCnt->GetBinContent(n));
  ...
  // // 	    // 	      analysis2->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuidePhotonEnergy((Float_t)LightGuideHit->GetPhotonEnergy()/eV);
  ...
  // // 	    // 	      wvl = 1259.842/(LightGuideHit->GetPhotonEnergy()/eV);
  ...
  if (eventNumber%10000 == 1) analysis5->AutoSaveRootNtuple();
  //=======================================================================
  
  
} // end of  MOLLEROptEventAction::EndOfEventAction()

void MOLLEROptEventAction5::Initialize()
{	  
  ...
  primaryEventNumber = 0;
    ...
    // 	    wvl = 1259.842/(PMTHit->GetPhotonEnergy()/eV);
    ...
    //   hitCnt5 = QuartzHC->entries();
    //   G4cout << "hitCnt5 = " << hitCnt5 << G4endl;
    //   if(hitCnt5){
	      
    // 	for(int j = 0; j < hitCnt5; j++){ //Note that each step inside a volume is a hit here    
    ...
    // 		ctrackID = trackID;
    // 		wvl = 1259.842/(QuartzHit->GetPhotonEnergy()/eV);
  ...
  // //     for(int n = 0; n < hitCnt5; n++){ //loop over quartz hits first
  ...
  // // 	      wvl = 1259.842/(QuartzHit->GetSecondaryPhotonEnergyAtIndex(c)/eV);
  // // 	      QuartzSecOptPhotonCnt->Fill(wvl,1.0/(bwdt));
  ...
  // //     //   QuartzTrackLength = 0;
  // // 	    //   QSteps = 0;
  // // 	    //   for(int j = 0; j < hitCnt5; j++){ //loop over quartz hits again, this time to add data for each secondary photon
