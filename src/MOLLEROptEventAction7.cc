...
MOLLEROptEventAction7::MOLLEROptEventAction7(MOLLEROptAnalysis7* AN, MOLLEROptTrackingReadout7* TrRO7)
{
  analysis7    = AN;
...
MOLLEROptEventAction7::~MOLLEROptEventAction7()
{
  oFile.close();
}


void MOLLEROptEventAction7::BeginOfEventAction(const G4Event* evt)
{
...
Bool_t MOLLEROptEventAction7::TrackHasBeenProcessed(G4int id)
{
  Bool_t check = false;
...
void MOLLEROptEventAction7::AddQuartzTrackStep(G4int id, G4int step)
{
  for(int n = 0; n < ProcessedTrack.size(); n++){
  ...
  lTrackData7 *data = new lTrackData7;
  data->ID = id;
  data->Length = step;
  data->Steps = 1;
...
void MOLLEROptEventAction7::EndOfEventAction(const G4Event* evt)
{

  Initialize();
  analysis7->MOLLERMainEvent->MOLLERPrimEvent.Initialize(); 
  ...
  G4int NumTracks = TrackingReadout7->GetNumTracks();
  ...
  for(int t = 0; t < NumTracks; t++){
    ...
    analysis7->MOLLERMainEvent->MOLLERDetectorEvent.Initialize();  

    track  = TrackingReadout7->GetTrackData7(t);
    if(track){

      analysis7->MOLLERMainEvent->MOLLERDetectorEvent.SetEventID(evt->GetEventID());
      analysis7->MOLLERMainEvent->MOLLERDetectorEvent.SetTrackParentID(track->ParentID);      
      analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddTrackInitMomDirection(track->InitMomDirX,track->InitMomDirY,
									      track->InitMomDirZ);      
      
      if(track->Particle == myElectron){
	analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddElectronTrackID(track->ID);
	analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddPhotonTrackID(0);

	if(track->QuartzHitFlag){
	  analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzTrackHit(1);
	  analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzHitPositionX((Float_t)track->QuartzHitX/cm);
	  analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzHitPositionY((Float_t)track->QuartzHitY/cm);
	}	
	...
	for(int p = 0; p < track->StepNChPhotons.size(); p++){
	  ...
	  analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzStepNPhotons(track->StepNChPhotons[p]);
	  analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzElectronStepLength(track->StepLength[p]/cm);
	}
	for(int p = 0; p < track->SecPhotonAngle.size(); p++)
	  analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzTrackSecPhotonAngle(track->SecPhotonAngle[p]);
      }
      if(track->Particle == myPhoton){
	analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddPhotonTrackID(track->ID);	
	analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddElectronTrackID(0);
	analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzTrackData(track->QLength/cm, track->QSteps);
	analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzPhotonEnergy(track->InitKinEnergy/eV);
	analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzPhotonAtExitFlag(track->QExitFlag);
	analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideTrackData(track->LGLength/cm, track->LGSteps);
	analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTTrackHit(track->PMTHitFlag);
	analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideTrackHit(track->LGHitFlag);
	analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTPhotonEnergy(track->InitKinEnergy/eV);
	analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuidePhotonEnergy(track->InitKinEnergy/eV);
	...
	if(track->PMTHitFlag){
	  ...
	  analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTHitPositionX((Float_t)track->PMTHitX/cm);
	  analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTHitPositionY((Float_t)track->PMTHitY/cm);
	  analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTWindowReflectionAngle((Float_t)track->PMTWinRefl);
	  analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTPhotonEnergy(track->InitKinEnergy/eV);
	  optPhEng = track->InitKinEnergy/eV;
	  OptParam* op = TrackingReadout7->GetOpticalParameters();
	...
	for(int s = 0; s < track->NSteps; s++){
	  if(track->StepVolume[s] == myQuartz){
	    analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzStepLength(track->StepLength[s]/cm);
	    analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzIncidentPhotonAngle(track->StepAngle[s]);
	  }
	  if(track->StepVolume[s] == myLightGuide){
	    analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideStepLength(track->StepLength[s]/cm);
	    analysis7->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideIncidentPhotonAngle(track->StepAngle[s]);
	  }
	}
	
      }
    }
    analysis7->FillRootNtuple();
  }

  analysis7->AddPhotoElectronEvent(PMTPe);
  analysis7->AddCathodeDetectionEvent(TrackingReadout7->GetCathodeDetections());
  
  for(int n = 1; n <= QuartzSecOptPhotonCnt->GetNbinsX(); n++)
    analysis7->AddToAverageQuartzOptPhotonDist(QuartzSecOptPhotonCnt->GetBinCenter(n),QuartzSecOptPhotonCnt->GetBinContent(n));
  for(int n = 1; n <= LightGuideSecOptPhotonCnt->GetNbinsX(); n++)
    analysis7->AddToAverageLightGuideOptPhotonDist(LightGuideSecOptPhotonCnt->GetBinCenter(n),LightGuideSecOptPhotonCnt->GetBinContent(n));
  for(int n = 1; n <= PMTSecOptPhotonCnt->GetNbinsX(); n++)
    analysis7->AddToAveragePMTOptPhotonDist(PMTSecOptPhotonCnt->GetBinCenter(n),PMTSecOptPhotonCnt->GetBinContent(n));
  ...
  // // 	    // 	      wvl = 1279.842/(LightGuideHit->GetPhotonEnergy()/eV);
  ...
  if (eventNumber%10000 == 1) analysis7->AutoSaveRootNtuple();
  //=======================================================================
  
  
} // end of  MOLLEROptEventAction::EndOfEventAction()

void MOLLEROptEventAction7::Initialize()
{	  
  ...
  primaryEventNumber = 0;
    ...
    // 	    wvl = 1279.842/(PMTHit->GetPhotonEnergy()/eV);
    ...
    //   hitCnt7 = QuartzHC->entries();
    //   G4cout << "hitCnt7 = " << hitCnt7 << G4endl;
    //   if(hitCnt7){
	      
    // 	for(int j = 0; j < hitCnt7; j++){ //Note that each step inside a volume is a hit here    
    ...
    // 		ctrackID = trackID;
    // 		wvl = 1279.842/(QuartzHit->GetPhotonEnergy()/eV);
  ...
  // //     for(int n = 0; n < hitCnt7; n++){ //loop over quartz hits first
  ...
  // // 	      wvl = 1279.842/(QuartzHit->GetSecondaryPhotonEnergyAtIndex(c)/eV);
  // // 	      QuartzSecOptPhotonCnt->Fill(wvl,1.0/(bwdt));
  ...
  // //     //   QuartzTrackLength = 0;
  // // 	    //   QSteps = 0;
  // // 	    //   for(int j = 0; j < hitCnt7; j++){ //loop over quartz hits again, this time to add data for each secondary photon
