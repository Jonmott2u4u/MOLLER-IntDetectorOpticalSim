...
MOLLEROptEventAction8::MOLLEROptEventAction8(MOLLEROptAnalysis8* AN, MOLLEROptTrackingReadout8* TrRO8)
{
  analysis8    = AN;
...
MOLLEROptEventAction8::~MOLLEROptEventAction8()
{
  oFile.close();
}


void MOLLEROptEventAction8::BeginOfEventAction(const G4Event* evt)
{
...
Bool_t MOLLEROptEventAction8::TrackHasBeenProcessed(G4int id)
{
  Bool_t check = false;
...
void MOLLEROptEventAction8::AddQuartzTrackStep(G4int id, G4int step)
{
  for(int n = 0; n < ProcessedTrack.size(); n++){
  ...
  lTrackData8 *data = new lTrackData8;
  data->ID = id;
  data->Length = step;
  data->Steps = 1;
...
void MOLLEROptEventAction8::EndOfEventAction(const G4Event* evt)
{

  Initialize();
  analysis8->MOLLERMainEvent->MOLLERPrimEvent.Initialize(); 
  ...
  G4int NumTracks = TrackingReadout8->GetNumTracks();
  ...
  for(int t = 0; t < NumTracks; t++){
    ...
    analysis8->MOLLERMainEvent->MOLLERDetectorEvent.Initialize();  

    track  = TrackingReadout8->GetTrackData8(t);
    if(track){

      analysis8->MOLLERMainEvent->MOLLERDetectorEvent.SetEventID(evt->GetEventID());
      analysis8->MOLLERMainEvent->MOLLERDetectorEvent.SetTrackParentID(track->ParentID);      
      analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddTrackInitMomDirection(track->InitMomDirX,track->InitMomDirY,
									      track->InitMomDirZ);      
      
      if(track->Particle == myElectron){
	analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddElectronTrackID(track->ID);
	analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddPhotonTrackID(0);

	if(track->QuartzHitFlag){
	  analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzTrackHit(1);
	  analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzHitPositionX((Float_t)track->QuartzHitX/cm);
	  analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzHitPositionY((Float_t)track->QuartzHitY/cm);
	}	
	...
	for(int p = 0; p < track->StepNChPhotons.size(); p++){
	  ...
	  analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzStepNPhotons(track->StepNChPhotons[p]);
	  analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzElectronStepLength(track->StepLength[p]/cm);
	}
	for(int p = 0; p < track->SecPhotonAngle.size(); p++)
	  analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzTrackSecPhotonAngle(track->SecPhotonAngle[p]);
      }
      if(track->Particle == myPhoton){
	analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddPhotonTrackID(track->ID);	
	analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddElectronTrackID(0);
	analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzTrackData(track->QLength/cm, track->QSteps);
	analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzPhotonEnergy(track->InitKinEnergy/eV);
	analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzPhotonAtExitFlag(track->QExitFlag);
	analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideTrackData(track->LGLength/cm, track->LGSteps);
	analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTTrackHit(track->PMTHitFlag);
	analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideTrackHit(track->LGHitFlag);
	analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTPhotonEnergy(track->InitKinEnergy/eV);
	analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuidePhotonEnergy(track->InitKinEnergy/eV);
	...
	if(track->PMTHitFlag){
	  ...
	  analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTHitPositionX((Float_t)track->PMTHitX/cm);
	  analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTHitPositionY((Float_t)track->PMTHitY/cm);
	  analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTWindowReflectionAngle((Float_t)track->PMTWinRefl);
	  analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddPMTPhotonEnergy(track->InitKinEnergy/eV);
	  optPhEng = track->InitKinEnergy/eV;
	  OptParam* op = TrackingReadout8->GetOpticalParameters();
	...
	for(int s = 0; s < track->NSteps; s++){
	  if(track->StepVolume[s] == myQuartz){
	    analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzStepLength(track->StepLength[s]/cm);
	    analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddQuartzIncidentPhotonAngle(track->StepAngle[s]);
	  }
	  if(track->StepVolume[s] == myLightGuide){
	    analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideStepLength(track->StepLength[s]/cm);
	    analysis8->MOLLERMainEvent->MOLLERDetectorEvent.AddLightGuideIncidentPhotonAngle(track->StepAngle[s]);
	  }
	}
	
      }
    }
    analysis8->FillRootNtuple();
  }

  analysis8->AddPhotoElectronEvent(PMTPe);
  analysis8->AddCathodeDetectionEvent(TrackingReadout8->GetCathodeDetections());
  
  for(int n = 1; n <= QuartzSecOptPhotonCnt->GetNbinsX(); n++)
    analysis8->AddToAverageQuartzOptPhotonDist(QuartzSecOptPhotonCnt->GetBinCenter(n),QuartzSecOptPhotonCnt->GetBinContent(n));
  for(int n = 1; n <= LightGuideSecOptPhotonCnt->GetNbinsX(); n++)
    analysis8->AddToAverageLightGuideOptPhotonDist(LightGuideSecOptPhotonCnt->GetBinCenter(n),LightGuideSecOptPhotonCnt->GetBinContent(n));
  for(int n = 1; n <= PMTSecOptPhotonCnt->GetNbinsX(); n++)
    analysis8->AddToAveragePMTOptPhotonDist(PMTSecOptPhotonCnt->GetBinCenter(n),PMTSecOptPhotonCnt->GetBinContent(n));
  ...
  // // 	    // 	      wvl = 1289.842/(LightGuideHit->GetPhotonEnergy()/eV);
  ...
  if (eventNumber%10000 == 1) analysis8->AutoSaveRootNtuple();
  //=======================================================================
  
  
} // end of  MOLLEROptEventAction::EndOfEventAction()

void MOLLEROptEventAction8::Initialize()
{	  
  ...
  primaryEventNumber = 0;
    ...
    // 	    wvl = 1289.842/(PMTHit->GetPhotonEnergy()/eV);
    ...
    //   hitCnt8 = QuartzHC->entries();
    //   G4cout << "hitCnt8 = " << hitCnt8 << G4endl;
    //   if(hitCnt8){
	      
    // 	for(int j = 0; j < hitCnt8; j++){ //Note that each step inside a volume is a hit here    
    ...
    // 		ctrackID = trackID;
    // 		wvl = 1289.842/(QuartzHit->GetPhotonEnergy()/eV);
  ...
  // //     for(int n = 0; n < hitCnt8; n++){ //loop over quartz hits first
  ...
  // // 	      wvl = 1289.842/(QuartzHit->GetSecondaryPhotonEnergyAtIndex(c)/eV);
  // // 	      QuartzSecOptPhotonCnt->Fill(wvl,1.0/(bwdt));
  ...
  // //     //   QuartzTrackLength = 0;
  // // 	    //   QSteps = 0;
  // // 	    //   for(int j = 0; j < hitCnt8; j++){ //loop over quartz hits again, this time to add data for each secondary photon
