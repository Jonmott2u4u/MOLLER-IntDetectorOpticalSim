...
MOLLEROptPMTSD8::~MOLLEROptPMTSD8()
{

}

void MOLLEROptPMTSD8::Initialize(G4HCofThisEvent* HCE)
{
  HitsCollection8 = new MOLLEROptPMTHitsCollection8(SensitiveDetectorName,collectionName[0]);
...
G4bool MOLLEROptPMTSD8::ProcessHits(G4Step* aStep, G4TouchableHistory* theTouchable)
{
	...
	for (G4int i=0; i<MAXofPostStepLoops; ++i) {
	  ...
	  if (opProc) {
	    ...
	    if(theStatus == FresnelReflection ||
	      ...
	      //TrackingReadout8->SetPMTWindowReflection(incidentAngle);
	  ...
	  TrackingReadout8->SetPMTHitLocation(aStep->GetTrack()->GetTrackID(),localpos,incidentAngle);
	  //HitsCollection8->insert(aHit); 
	// }
      // }
...
void MOLLEROptPMTSD8::EndOfEvent(G4HCofThisEvent* )
{
