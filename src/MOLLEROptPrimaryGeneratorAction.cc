...
*/
  if(EventRegion == 1){
    ...
    y = (Qlim1[2] + 1396*TMath::Sin(6*pi/180)) + (Qlim8[3]-Qlim1[2]-1396*TMath::Sin(6*pi/180))*G4UniformRand();
  }
    ...
    x = (Qlim1[1]+Qlim1[0])/2.0 -2 +4*G4UniformRand();
    y = Qlim1[2] + 1396*TMath::Sin(6*pi/180) + (cut-5*G4UniformRand()); //cut has units cm, whereas the rest has units mm
  }
    ...
    y = y_base + 1769*TMath::Sin(6*pi/180);
  ...
  G4double cosTheta = TMath::Cos(G4UniformRand()*sa_rad); //Makes the angle of the beam random within +- sa
