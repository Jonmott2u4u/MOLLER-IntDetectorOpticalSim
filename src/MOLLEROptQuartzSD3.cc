#include "MOLLEROptQuartzSD3.hh"

MOLLEROptQuartzSD3::MOLLEROptQuartzSD3(G4String name, MOLLEROptTrackingReadout3* TrRO)
...
MOLLEROptQuartzSD3::~MOLLEROptQuartzSD3()
{

}

void MOLLEROptQuartzSD3::Initialize(G4HCofThisEvent* HCE)
{
  HitsCollection3 = new MOLLEROptQuartzHitsCollection3(SensitiveDetectorName,collectionName[0]);
...
G4bool MOLLEROptQuartzSD3::ProcessHits(G4Step* aStep, G4TouchableHistory* theTouchable)
{
  G4int QEx = 0;
  G4int nsec = 0;
  ...
  MOLLEROptQuartzHit3* aHit = new MOLLEROptQuartzHit3();
  ...
  if(aStep->GetTrack()->GetDefinition() == G4OpticalPhoton::OpticalPhotonDefinition()){
	...
	if(vol){
	  ...
	  if(postStep->GetPhysicalVolume()->GetName().contains("LG_Physical") &&
	    ...
	    // G4cout << "Exit after step: " << TrackingReadout3->GetStepInQuartz(aStep->GetTrack()->GetTrackID()) << G4endl;
	...
	TrackingReadout3->AddTrackData(aStep->GetTrack()->GetTrackID(),myPhoton,
  ...
  if(aStep->GetTrack()->GetDefinition() == G4Electron::ElectronDefinition()){
    ...
    for(int n = 0; n < (*Secondaries).size(); n++){
      ...
      // 	// TrackingReadout3->AddTrackData(aStep->GetTrack()->GetTrackID(),myElectron,
	...
	TrackingReadout3->AddSecPhoton(aStep->GetTrack()->GetTrackID(),Angle,1239.842/((*Secondaries)[n]->GetTotalEnergy()/eV));
    ...
    TrackingReadout3->AddTrackData(aStep->GetTrack()->GetTrackID(),myElectron,
				  aStep->GetStepLength(),0,0,myQuartz,0,
				  aStep->GetTrack()->GetKineticEnergy(),0,0);
    TrackingReadout3->AddStepNCherenkovs(aStep->GetTrack()->GetTrackID(),nsec);
    TrackingReadout3->SetQuartzHitLocation(aStep->GetTrack()->GetTrackID(),worldpos);
...
void MOLLEROptQuartzSD3::EndOfEvent(G4HCofThisEvent* )
{
