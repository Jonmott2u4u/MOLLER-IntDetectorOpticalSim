#include "MOLLEROptQuartzSD4.hh"

MOLLEROptQuartzSD4::MOLLEROptQuartzSD4(G4String name, MOLLEROptTrackingReadout4* TrRO4)
...
MOLLEROptQuartzSD4::~MOLLEROptQuartzSD4()
{

}

void MOLLEROptQuartzSD4::Initialize(G4HCofThisEvent* HCE)
{
  HitsCollection4 = new MOLLEROptQuartzHitsCollection4(SensitiveDetectorName,collectionName[0]);
...
G4bool MOLLEROptQuartzSD4::ProcessHits(G4Step* aStep, G4TouchableHistory* theTouchable)
{
  G4int QEx = 0;
  G4int nsec = 0;
  ...
  MOLLEROptQuartzHit4* aHit = new MOLLEROptQuartzHit4();
  ...
  if(aStep->GetTrack()->GetDefinition() == G4OpticalPhoton::OpticalPhotonDefinition()){
	...
	if(vol){
	  ...
	  if(postStep->GetPhysicalVolume()->GetName().contains("LG_Physical") &&
	    ...
	    // G4cout << "Exit after step: " << TrackingReadout4->GetStepInQuartz(aStep->GetTrack()->GetTrackID()) << G4endl;
	...
	TrackingReadout4->AddTrackData(aStep->GetTrack()->GetTrackID(),myPhoton,
  ...
  if(aStep->GetTrack()->GetDefinition() == G4Electron::ElectronDefinition()){
    ...
    for(int n = 0; n < (*Secondaries).size(); n++){
      ...
      // 	// TrackingReadout4->AddTrackData(aStep->GetTrack()->GetTrackID(),myElectron,
	...
	TrackingReadout4->AddSecPhoton(aStep->GetTrack()->GetTrackID(),Angle,1239.842/((*Secondaries)[n]->GetTotalEnergy()/eV));
    ...
    TrackingReadout4->AddTrackData(aStep->GetTrack()->GetTrackID(),myElectron,
				  aStep->GetStepLength(),0,0,myQuartz,0,
				  aStep->GetTrack()->GetKineticEnergy(),0,0);
    TrackingReadout4->AddStepNCherenkovs(aStep->GetTrack()->GetTrackID(),nsec);
    TrackingReadout4->SetQuartzHitLocation(aStep->GetTrack()->GetTrackID(),worldpos);
...
void MOLLEROptQuartzSD4::EndOfEvent(G4HCofThisEvent* )
{
