#include "MOLLEROptQuartzSD5.hh"

MOLLEROptQuartzSD5::MOLLEROptQuartzSD5(G4String name, MOLLEROptTrackingReadout5* TrRO5)
...
MOLLEROptQuartzSD5::~MOLLEROptQuartzSD5()
{

}

void MOLLEROptQuartzSD5::Initialize(G4HCofThisEvent* HCE)
{
  HitsCollection5 = new MOLLEROptQuartzHitsCollection5(SensitiveDetectorName,collectionName[0]);
...
G4bool MOLLEROptQuartzSD5::ProcessHits(G4Step* aStep, G4TouchableHistory* theTouchable)
{
  G4int QEx = 0;
  G4int nsec = 0;
  ...
  MOLLEROptQuartzHit5* aHit = new MOLLEROptQuartzHit5();
  ...
  if(aStep->GetTrack()->GetDefinition() == G4OpticalPhoton::OpticalPhotonDefinition()){
	...
	if(vol){
	  ...
	  if(postStep->GetPhysicalVolume()->GetName().contains("LG_Physical") &&
	    ...
	    // G4cout << "Exit after step: " << TrackingReadout5->GetStepInQuartz(aStep->GetTrack()->GetTrackID()) << G4endl;
	...
	TrackingReadout5->AddTrackData(aStep->GetTrack()->GetTrackID(),myPhoton,
  ...
  if(aStep->GetTrack()->GetDefinition() == G4Electron::ElectronDefinition()){
    ...
    for(int n = 0; n < (*Secondaries).size(); n++){
      ...
      // 	// TrackingReadout5->AddTrackData(aStep->GetTrack()->GetTrackID(),myElectron,
	...
	TrackingReadout5->AddSecPhoton(aStep->GetTrack()->GetTrackID(),Angle,1239.842/((*Secondaries)[n]->GetTotalEnergy()/eV));
    ...
    TrackingReadout5->AddTrackData(aStep->GetTrack()->GetTrackID(),myElectron,
				  aStep->GetStepLength(),0,0,myQuartz,0,
				  aStep->GetTrack()->GetKineticEnergy(),0,0);
    TrackingReadout5->AddStepNCherenkovs(aStep->GetTrack()->GetTrackID(),nsec);
    TrackingReadout5->SetQuartzHitLocation(aStep->GetTrack()->GetTrackID(),worldpos);
...
void MOLLEROptQuartzSD5::EndOfEvent(G4HCofThisEvent* )
{
