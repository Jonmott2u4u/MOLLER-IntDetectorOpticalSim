#include "MOLLEROptQuartzSD6.hh"

MOLLEROptQuartzSD6::MOLLEROptQuartzSD6(G4String name, MOLLEROptTrackingReadout6* TrRO6)
...
MOLLEROptQuartzSD6::~MOLLEROptQuartzSD6()
{

}

void MOLLEROptQuartzSD6::Initialize(G4HCofThisEvent* HCE)
{
  HitsCollection6 = new MOLLEROptQuartzHitsCollection6(SensitiveDetectorName,collectionName[0]);
...
G4bool MOLLEROptQuartzSD6::ProcessHits(G4Step* aStep, G4TouchableHistory* theTouchable)
{
  G4int QEx = 0;
  G4int nsec = 0;
  ...
  MOLLEROptQuartzHit6* aHit = new MOLLEROptQuartzHit6();
  ...
  if(aStep->GetTrack()->GetDefinition() == G4OpticalPhoton::OpticalPhotonDefinition()){
	...
	if(vol){
	  ...
	  if(postStep->GetPhysicalVolume()->GetName().contains("LG_Physical") &&
	    ...
	    // G4cout << "Exit after step: " << TrackingReadout6->GetStepInQuartz(aStep->GetTrack()->GetTrackID()) << G4endl;
	...
	TrackingReadout6->AddTrackData(aStep->GetTrack()->GetTrackID(),myPhoton,
  ...
  if(aStep->GetTrack()->GetDefinition() == G4Electron::ElectronDefinition()){
    ...
    for(int n = 0; n < (*Secondaries).size(); n++){
      ...
      // 	// TrackingReadout6->AddTrackData(aStep->GetTrack()->GetTrackID(),myElectron,
	...
	TrackingReadout6->AddSecPhoton(aStep->GetTrack()->GetTrackID(),Angle,1239.842/((*Secondaries)[n]->GetTotalEnergy()/eV));
    ...
    TrackingReadout6->AddTrackData(aStep->GetTrack()->GetTrackID(),myElectron,
				  aStep->GetStepLength(),0,0,myQuartz,0,
				  aStep->GetTrack()->GetKineticEnergy(),0,0);
    TrackingReadout6->AddStepNCherenkovs(aStep->GetTrack()->GetTrackID(),nsec);
    TrackingReadout6->SetQuartzHitLocation(aStep->GetTrack()->GetTrackID(),worldpos);
...
void MOLLEROptQuartzSD6::EndOfEvent(G4HCofThisEvent* )
{
