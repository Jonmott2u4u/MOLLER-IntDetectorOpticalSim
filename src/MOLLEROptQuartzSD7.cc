#include "MOLLEROptQuartzSD7.hh"

MOLLEROptQuartzSD7::MOLLEROptQuartzSD7(G4String name, MOLLEROptTrackingReadout7* TrRO7)
...
MOLLEROptQuartzSD7::~MOLLEROptQuartzSD7()
{

}

void MOLLEROptQuartzSD7::Initialize(G4HCofThisEvent* HCE)
{
  HitsCollection7 = new MOLLEROptQuartzHitsCollection7(SensitiveDetectorName,collectionName[0]);
...
G4bool MOLLEROptQuartzSD7::ProcessHits(G4Step* aStep, G4TouchableHistory* theTouchable)
{
  G4int QEx = 0;
  G4int nsec = 0;
  ...
  MOLLEROptQuartzHit7* aHit = new MOLLEROptQuartzHit7();
  ...
  if(aStep->GetTrack()->GetDefinition() == G4OpticalPhoton::OpticalPhotonDefinition()){
	...
	if(vol){
	  ...
	  if(postStep->GetPhysicalVolume()->GetName().contains("LG_Physical") &&
	    ...
	    // G4cout << "Exit after step: " << TrackingReadout7->GetStepInQuartz(aStep->GetTrack()->GetTrackID()) << G4endl;
  ...
  if(aStep->GetTrack()->GetDefinition() == G4Electron::ElectronDefinition()){
    ...
    for(int n = 0; n < (*Secondaries).size(); n++){
      ...
      // 	// TrackingReadout7->AddTrackData(aStep->GetTrack()->GetTrackID(),myElectron,
	...
	TrackingReadout7->AddSecPhoton(aStep->GetTrack()->GetTrackID(),Angle,1239.842/((*Secondaries)[n]->GetTotalEnergy()/eV));
    ...
    TrackingReadout7->AddTrackData(aStep->GetTrack()->GetTrackID(),myElectron,
				  aStep->GetStepLength(),0,0,myQuartz,0,
				  aStep->GetTrack()->GetKineticEnergy(),0,0);
    TrackingReadout7->AddStepNCherenkovs(aStep->GetTrack()->GetTrackID(),nsec);
    TrackingReadout7->SetQuartzHitLocation(aStep->GetTrack()->GetTrackID(),worldpos);
...
void MOLLEROptQuartzSD7::EndOfEvent(G4HCofThisEvent* )
{
