...
MOLLEROptTrackingReadout2::MOLLEROptTrackingReadout2()
{
  Initialize();
  QAbsProf.clear();
  QAbsProf.resize(0);
...
void MOLLEROptTrackingReadout2::AddTrackData(Int_t id, Int_t type, Int_t StepL, Int_t QEFlag, Int_t PMTHit, Int_t Volume, Int_t Proc, Float_t eng, Float_t wvl, Float_t Ang)
{
  Int_t tr = -1;
...
void MOLLEROptTrackingReadout2::AddStepNCherenkovs(Int_t id, Int_t nsec)
{ 
  for(int n = 0; n < Tracks.size(); n++)
    if(Tracks[n]->ID == id){
...
void MOLLEROptTrackingReadout2::IncrementEventCathodeDetection(Int_t id)
{
...
void MOLLEROptTrackingReadout2::AddSecPhoton(Int_t id, Float_t ang, Float_t wvl)
{
...
void MOLLEROptTrackingReadout2::SetPMTHitLocation(Int_t id, G4ThreeVector loc, Float_t angle)
{
...
void MOLLEROptTrackingReadout2::SetQuartzHitLocation(Int_t id, G4ThreeVector loc)
{
...
void MOLLEROptTrackingReadout2::AddNewTrack(TrackData2 *track)
{
...
Bool_t MOLLEROptTrackingReadout2::TrackExists(Int_t id)
{
  for(int n = 0; n < Tracks.size(); n++)
...
Int_t MOLLEROptTrackingReadout2::GetStepInQuartz(Int_t TID)
{
  for(int n = 0; n < Tracks.size(); n++){
    if(Tracks[n]->ID == TID){
...
void MOLLEROptTrackingReadout2::StoreQuartzOpticalPlots(TProfile* tr35, TProfile* tr70, TProfile* tr170, TProfile* refl, TH1D* Qeff)
{
...
void MOLLEROptTrackingReadout2::StoreGuideOpticalPlots(TH1D* rl30,TH1D* rl45,TH1D* rl60,TH1D* rl90)
{
...
void MOLLEROptTrackingReadout2::StoreCathodeOpticalPlots(TH1D* CIndR, TH1D* CIndI)
{
...
void MOLLEROptTrackingReadout2:: SetRadiationDamageHistograms(TH2 *hst, TH2 *hst2, TH2 *hst3, TH2 *hst4)
{
...
void MOLLEROptTrackingReadout2::SetNewQAbsorptionProfile(TProfile* abs)
{
  QAbsProf.push_back(abs);
}

void MOLLEROptTrackingReadout2::WriteAbsProfiles()
{
...
void MOLLEROptTrackingReadout2::Initialize()
{
