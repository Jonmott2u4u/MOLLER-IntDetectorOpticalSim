...
MOLLEROptTrackingReadout3::MOLLEROptTrackingReadout3()
{
  Initialize();
  QAbsProf.clear();
  QAbsProf.resize(0);
...
void MOLLEROptTrackingReadout3::AddTrackData(Int_t id, Int_t type, Int_t StepL, Int_t QEFlag, Int_t PMTHit, Int_t Volume, Int_t Proc, Float_t eng, Float_t wvl, Float_t Ang)
{
  Int_t tr = -1;
...
void MOLLEROptTrackingReadout3::AddStepNCherenkovs(Int_t id, Int_t nsec)
{ 
  for(int n = 0; n < Tracks.size(); n++)
    if(Tracks[n]->ID == id){
...
void MOLLEROptTrackingReadout3::IncrementEventCathodeDetection(Int_t id)
{
...
void MOLLEROptTrackingReadout3::AddSecPhoton(Int_t id, Float_t ang, Float_t wvl)
{
...
void MOLLEROptTrackingReadout3::SetPMTHitLocation(Int_t id, G4ThreeVector loc, Float_t angle)
{
...
void MOLLEROptTrackingReadout3::SetQuartzHitLocation(Int_t id, G4ThreeVector loc)
{
...
void MOLLEROptTrackingReadout3::AddNewTrack(TrackData3 *track)
{
...
Bool_t MOLLEROptTrackingReadout3::TrackExists(Int_t id)
{
  for(int n = 0; n < Tracks.size(); n++)
...
Int_t MOLLEROptTrackingReadout3::GetStepInQuartz(Int_t TID)
{
  for(int n = 0; n < Tracks.size(); n++){
    if(Tracks[n]->ID == TID){
...
void MOLLEROptTrackingReadout3::StoreQuartzOpticalPlots(TProfile* tr35, TProfile* tr70, TProfile* tr170, TProfile* refl, TH1D* Qeff)
{
...
void MOLLEROptTrackingReadout3::StoreGuideOpticalPlots(TH1D* rl30,TH1D* rl45,TH1D* rl60,TH1D* rl90)
{
...
void MOLLEROptTrackingReadout3::StoreCathodeOpticalPlots(TH1D* CIndR, TH1D* CIndI)
{
...
void MOLLEROptTrackingReadout3:: SetRadiationDamageHistograms(TH2 *hst, TH2 *hst2, TH2 *hst3, TH2 *hst4)
{
...
void MOLLEROptTrackingReadout3::SetNewQAbsorptionProfile(TProfile* abs)
{
  QAbsProf.push_back(abs);
}

void MOLLEROptTrackingReadout3::WriteAbsProfiles()
{
...
void MOLLEROptTrackingReadout3::Initialize()
{
