...
MOLLEROptTrackingReadout5::MOLLEROptTrackingReadout5()
{
  Initialize();
  QAbsProf.clear();
  QAbsProf.resize(0);
...
void MOLLEROptTrackingReadout5::AddTrackData(Int_t id, Int_t type, Int_t StepL, Int_t QEFlag, Int_t PMTHit, Int_t Volume, Int_t Proc, Float_t eng, Float_t wvl, Float_t Ang)
{
  Int_t tr = -1;
...
void MOLLEROptTrackingReadout5::AddStepNCherenkovs(Int_t id, Int_t nsec)
{ 
  for(int n = 0; n < Tracks.size(); n++)
    if(Tracks[n]->ID == id){
...
void MOLLEROptTrackingReadout5::IncrementEventCathodeDetection(Int_t id)
{
...
void MOLLEROptTrackingReadout5::AddSecPhoton(Int_t id, Float_t ang, Float_t wvl)
{
...
void MOLLEROptTrackingReadout5::SetPMTHitLocation(Int_t id, G4ThreeVector loc, Float_t angle)
{
...
void MOLLEROptTrackingReadout5::SetQuartzHitLocation(Int_t id, G4ThreeVector loc)
{
...
void MOLLEROptTrackingReadout5::AddNewTrack(TrackData5 *track)
{
...
Bool_t MOLLEROptTrackingReadout5::TrackExists(Int_t id)
{
  for(int n = 0; n < Tracks.size(); n++)
...
Int_t MOLLEROptTrackingReadout5::GetStepInQuartz(Int_t TID)
{
  for(int n = 0; n < Tracks.size(); n++){
    if(Tracks[n]->ID == TID){
...
void MOLLEROptTrackingReadout5::StoreQuartzOpticalPlots(TProfile* tr35, TProfile* tr70, TProfile* tr170, TProfile* refl, TH1D* Qeff)
{
...
void MOLLEROptTrackingReadout5::StoreGuideOpticalPlots(TH1D* rl30,TH1D* rl45,TH1D* rl60,TH1D* rl90)
{
...
void MOLLEROptTrackingReadout5::StoreCathodeOpticalPlots(TH1D* CIndR, TH1D* CIndI)
{
...
void MOLLEROptTrackingReadout5:: SetRadiationDamageHistograms(TH2 *hst, TH2 *hst2, TH2 *hst3, TH2 *hst4)
{
...
void MOLLEROptTrackingReadout5::SetNewQAbsorptionProfile(TProfile* abs)
{
  QAbsProf.push_back(abs);
}

void MOLLEROptTrackingReadout5::WriteAbsProfiles()
{
...
void MOLLEROptTrackingReadout5::Initialize()
{
